...
keywords: ["x402 server", "accept agent payments", "monetize API agent", "x402-express", "monetize-service skill", "Base agent monetization"]
---
...
## Option 1 — `monetize-service` skill (simplest)
...
## Option 2 — Custom server with `x402-express`
...
## Option 3 — `Hono` and other frameworks

x402 ships middleware for `hono`, `next`, and `fastify` with the same configuration shape. See the [x402 server reference](https://docs.cdp.coinbase.com/x402/docs/client-server-model) for framework-specific examples.
